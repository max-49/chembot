...
class Events(commands.Cog):
    ...
    async def addevent(self, ctx):
        ...
        await ctx.send(f"{ctx.author.mention}, please enter the date of this event in the format MM/DD/YYYY (i.e. for the date of April 24th, 2023, enter 04/24/2023)")
        date = await self.bot.wait_for("message", check=check)
        await ctx.send(f"{ctx.author.mention}, please enter the time of this event IN MILITARY TIME in the format HH:MM (i.e. for the time of 10:30pm, enter 22:30)")
        event_time = await self.bot.wait_for("message", check=check)
        await ctx.send(f"{ctx.author.mention}, what is the event? (Please enter the message you want to be pinged with when this event happens)")
        message = await self.bot.wait_for("message", check=check)
        await ctx.send(f"{ctx.author.mention}, what channel do you want this message to be sent in? Please mention it.")
        channel = await self.bot.wait_for("message", check=check)
        await ctx.send(f"{ctx.author.mention}, would you like to be pinged for this event (y/n)?")
        user_ping = await self.bot.wait_for("message", check=check_yn)
        await ctx.send(f"{ctx.author.mention}, would you like another role/person to be pinged for this event (y/n)?")
        other_ping = await self.bot.wait_for("message", check=check_yn)
        other_message = ""
        if (other_ping.content == 'y'):
            await ctx.send(f"{ctx.author.mention}, please enter all the role/user pings that should be pinged when this event occurs, separated by commas.")

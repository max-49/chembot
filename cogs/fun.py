...
class Fun(commands.Cog):
    ...
    async def say(self, ctx, *, message: str):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            await webhook.send(message, username=ctx.author.display_name, avatar_url=ctx.author.avatar_url, allowed_mentions=discord.AllowedMentions(users=False))
    ...
    async def saym(self, ctx, member: discord.Member, *, message: str):
        ...
        async with aiohttp.ClientSession() as session:
            webhook = Webhook.from_url(
                use_hook.url, adapter=AsyncWebhookAdapter(session))
            await webhook.send(message, username=member.display_name, avatar_url=member.avatar_url, allowed_mentions=discord.AllowedMentions(users=True))
